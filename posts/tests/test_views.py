...
class PostsTestViews(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.post = Post.objects.create(
            text="Тестовый пост 1", author=cls.user, group=cls.group_1
        )
    ...
    def test_loading_not_images(self):
        ...
        image = SimpleUploadedFile(
            name="admin.py", content=open("posts/admin.py", "rb").read()
        )
        context = {
            "text": "Это текст публикации",
            "group": self.group_1.id,
        ...
        response = self.authorized_client.post(
            reverse("new_post"), context, follow=True
        )
    ...
    def test_comment_authorized_user(self):
        ...
        response = self.authorized_client.post(
            url, {"text": "Это текст комментария"}, follow=True
        )
        self.assertEqual(response.status_code, 200)
